...
\newcommand{\highlight}[1]{%
    ...
    \begin{itemize} \itemsep1.5em
            \item platform $P$ with its surface gridded such that end points represent possible antenna placement
            \item set of  $m~(m > 1)$ antennas $A = {A_1, A_2, \dots, A_m}$
            \item for each $A_i$, let $L_i$ denote the set of allowable placements locations $\in \mathbb R^3$ such that $\mid L_i \mid = n_i$ and $\forall i, n_i > 1$; $L_i = \{(x_{1}, y_{1}, z_{1}) \dots (x_{n_i}, y_{n_i}, z_{n_i})\}$
        \end{itemize}
        \vspace{10px}
        \centering\only{\textbf{Problem}: Find a set of $n$ optimal antenna locations on $P$}
    \end{frame}

    \begin{frame}{\null}
        \begin{tcolorbox}[colback=green!5]
            \centering
            Question: How is a good placement antenna placement quantified in context of platform and other antennas?
        \end{tcolorbox}
    \end{frame}
    ...
    \begin{frame}[t]{Mutual Coupling}
        ...
        \begin{figure}\centering
            \begin{tikzpicture} [
                ...
                \node (b1) at (0, 1) [draw,thick,minimum width=.75cm,minimum height=.75cm, label=below:Antenna $n$] {}; 
                \node[triangle, border rotated, above=1cm of b1] (a1) {};
                \draw [-] (a1) -- (b1);
                \node (b2) at (5, 1) [draw,thick,minimum width=.75cm,minimum height=.75cm, label=below:Antenna $m$] {}; 
                \node[triangle, border rotated, above=1cm of b2] (a2) {};
                \draw [-] (a2) -- (b2);
                \draw [->] ([xshift=1cm] b2.south) to node[right] {\scriptsize $B_{in}$} ([xshift=1cm] b2.north);
                \draw [<-, shorten <= 1cm, shorten >= 1cm] (a1.north) to[bend left] node {\scriptsize $B_c$} (a2.north);
                ...
                \node[draw, text width=3.5cm] at (8,2) {\tiny $B_{in}~$:  Energy from generator\\$B_{out}$: Energy radiated in space\\{\color{red}$B_c~~$: Coupled energy (not good)}\\$A_{out}$: Re-radiated energy\\}; 
            \end{tikzpicture}
        \end{figure}
    \end{frame}
