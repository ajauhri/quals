...
\tikzset{antenna/.style={insert path={-- coordinate (ant#1) ++(0,3.25)}}}
\tikzset{radiation/.style={{decorate,decoration={expanding waves,angle=90,segment length=4pt}}}}

\newcommand{\BS}[1]{%
\begin{tikzpicture}
\node[] (base) {};
\draw[line cap=rect] ([yshift=.6pt]base.north) [antenna=1];
\end{tikzpicture}
}
...
\newcommand{\highlight}[1]{%
    ...
    \begin{frame}[t]{How is a good antenna placement defined?}
        Minimize energy absorbed by one antenna's receiver when another antenna operating nearby. \vspace{2mm}
        \begin{columns}
            \column{0.45\linewidth}
        \begin{tikzpicture}
            \node[label=below:$\text{No coupling}$,every path/.append style={thick},inner sep=0pt](A){\BS{\textbf{1}}}; 
            \draw[radiation] (A.north) -- +(90:0.9);
        \end{tikzpicture}
        \column{0.45\linewidth}
        \begin{tikzpicture}
            [align=left,node distance=2.1cm]
            \node[every path/.append style={thick},inner sep=0pt](A){\BS{\textbf{1}}}; 
            \node[every path/.append style={thick},inner sep=0pt, right of=A](B){\BS{\textbf{1}}}; 
            \node at (1,-2) {Mutual Coupling};
            \draw[radiation] (A.north) -- +(90:0.9);
            \draw[radiation] (B.north) -- +(90:0.9);
        \end{tikzpicture}
    \end{columns}
    \end{frame}
