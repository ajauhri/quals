...
\begin{overlayarea}{\textwidth}{\textheight}
            \begin{figure}
                ...
                \begin{subfigure}{\columnwidth}
                    \centering
                        \caption*{\tiny {\color{red}Pattern with platform and antennas (in-situ)}}%
                        \includegraphics[width=0.5\columnwidth,height=0.5\columnwidth]{../paper/FIG/rand.png}
                \end{subfigure}\\\vfill
                \only<2>{
                \begin{subfigure}{\columnwidth}
                    \centering
                    \caption*{\tiny {\color{blue}Pattern with platform and antennas (in-situ) similar to free-space pattern}}%
...
\begin{frame}[t]{Stochastic Algorithms: Operand}
    \textbf{Candidate solution} or an \textbf{individual} is a member of a set of possible olutions.
\begin{itemize} \itemsep1.5em
    ...
    \begin{columns}
        \begin{column}{0.66\linewidth}%added for this algorithm since text wasn't fitting in one slide
            \begin{spacing}{0.6}
                ...
                \begin{algorithm}[H]
                    \only<1>{\reduline{\textbf{P} $\leftarrow$ generate $p$ random individuals. Compute $fitness(ind_i), i \in [1,p]$}\;}
                    ...
                    \While{$i<gen_{max}$} {
                        ...
                        \For{$(p - n_e)/2$ times} {
                            ...
                            \eIf {$rand(0,1)<p_c$} {
                                ...
                                \only<2>{\reduline{Apply $crossover(M)$ to get two offsprings \textbf{O}} \;}
                                Add $O$ to $P'$ \;
                            }
                            { Add $M$ to $P'$ \; }
                        }
        ...
        \begin{column}{0.66\linewidth}
            ...
            \begin{algorithm}[H]
                ...
                \While{$i<gen_{max}$} {
                    ...
                    \only<2>{\reduline{Create $\lambda / \mu$ offsprings from  each $\mu$ individuals by applying $mutation$ operator, and add all offsprings to \textbf{P}} \;}
